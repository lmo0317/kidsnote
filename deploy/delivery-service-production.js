...
function startDeliveryService(app, options) {
  ...
  function publicRequest(item) {
    return { id: item.id, email: item.email, year: item.year, deliveryNote: item.deliveryNote || '', includeNotices: item.includeNotices === true, folderMode: item.folderMode || 'day', status: item.status, createdAt: item.createdAt, updatedAt: item.updatedAt, jobId: item.jobId || '', error: item.error || '', canRerun: Boolean(item.encryptedCredentials) };
  }
  ...
  function publicJob(job) {
    return {
      ...
      includeNotices: job.includeNotices === true, folderMode: job.folderMode || 'day',
  ...
  async function makeZip(job, filesDir) {
    const zipPath = path.join(jobsDir, job.id, `kidsnote-${job.year || 'all'}.zip`);
  ...
  function publicFileUrl(pathname) {
    const configured = String(process.env.PUBLIC_BASE_URL || '').trim();
    if (!configured) return `/kidsnote-files${pathname}`;
    let origin;
    try {
      origin = new URL(configured).origin;
    } catch {
      throw new Error('PUBLIC_BASE_URL 설정이 올바른 URL이 아닙니다.');
    }
    return `${origin}/kidsnote-files${pathname}`;
  }

  function shareUrls(job, tokens) {
    return {
      downloadUrl: publicFileUrl(`/delivery/open/${encodeURIComponent(job.id)}/${encodeURIComponent(tokens.downloadToken)}`),
      deleteUrl: publicFileUrl(`/delivery/delete/${encodeURIComponent(job.id)}/${encodeURIComponent(tokens.deleteToken)}`)
    };
  }
  ...
  async function processJob(job, username, password) {
    ...
    try {
      ...
      update(job, { status: 'collecting', progress: { ...job.progress, step: job.includeNotices ? '사진과 알림장 목록 수집 중' : '사진 목록 수집 중' } });
      ...
      for (const child of children) {
        ...
        for (const collection of [{ name: 'reports', type: 'report' }, { name: 'albums', type: 'album' }]) {
          ...
          update(job, { progress: { ...job.progress, found: candidates.size } });
        }
      }
      if (!candidates.size && !noticeCandidates.length) throw new Error('선택한 기간에 저장할 사진이나 알림장이 없습니다.');
      update(job, { status: 'downloading', progress: { ...job.progress, step: '사진 다운로드 중', found: candidates.size } });
      const usedNames = new Map(); let totalSize = 0;
      const candidatesList = [...candidates.entries()];
      const CONCURRENCY = 16;
      let currentIndex = 0;
      let lastProgressUpdate = 0;

      async function worker() {
        while (currentIndex < candidatesList.length) {
          ensureNotCancelled(job);
          const index = currentIndex++;
          const [sourceUrl, meta] = candidatesList[index];
          try {
          ...
          if (totalProcessed - lastProgressUpdate >= 20 || totalProcessed === candidatesList.length) {
      ...
      await Promise.all(Array.from({ length: Math.min(CONCURRENCY, candidatesList.length) }, () => worker()));
      if (candidates.size && !job.progress.downloaded && !noticeCandidates.length) throw new Error('사진을 다운로드하지 못했습니다.');
      const noticeCount = job.includeNotices ? writeNoticeFiles(noticeCandidates, filesDir, job.folderMode) : 0;
      ensureNotCancelled(job);
      ...
      job.zipPath = zipPath;
      job.result = { count: job.progress.downloaded, noticeCount, failed: job.progress.failed, size: fs.statSync(zipPath).size, emailSent: false, breakdown: job.progress.breakdown };
  ...
  app.post('/api/delivery/requests', (req, res) => {
    ...
    const now = Date.now();
    const item = { id: crypto.randomBytes(12).toString('hex'), email, year, deliveryNote, includeNotices, folderMode, status: 'pending', createdAt: now, updatedAt: now, encryptedCredentials: encryptCredentials({ username, password }) };
  ...
  const handleOpen = (req, res) => {
    ...
    const fileUrl = publicFileUrl(`/delivery/download/${encodeURIComponent(req.params.jobId)}/${encodeURIComponent(req.params.token)}`);
  ...
  app.get('/delivery/open/:jobId/:token', handleOpen);
  app.get('/kidsnote-files/delivery/open/:jobId/:token', handleOpen);
  app.get('/delivery/download/:jobId/:token', handleDownload);
  app.get('/kidsnote-files/delivery/download/:jobId/:token', handleDownload);
  app.get('/delivery/delete/:jobId/:token', handleDeleteGet);
  app.get('/kidsnote-files/delivery/delete/:jobId/:token', handleDeleteGet);
  app.post('/delivery/delete/:jobId/:token', handleDeletePost);
  app.post('/kidsnote-files/delivery/delete/:jobId/:token', handleDeletePost);
